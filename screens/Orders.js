...
function Orders({ navigation }) {
  ...
  useEffect(() => {
    if (config.DRIVER_APP) {
      ...
      API.getVendorOrders((ordersResponse) => {
        ...
        toastok.current.show(Language.today_order, 3000, () => { })
    ...
    if (config.DRIVER_APP) {
      if (initValue) {
        return (
          <Block row={true} card flex style={styles.actioncard} >
            <Block style={{ "some": available }} flex space="between" justifyContent="center" paddingHorizontal={10} >
              <Checkbox key={"true"} onChange={setActiveStatus} initialValue={true} color="success" labelStyle={{ color: "white" }} checkboxStyle={{ margin: 10 }} label={Language.driverWorkingStatus} />
            </Block>
          </Block>
        )
      } else {
        return (
          <Block row={true} card flex style={styles.actioncard} >
            <Block style={{ "some": available }} flex space="between" justifyContent="center" paddingHorizontal={10} >
              <Checkbox key={"false"} onChange={setActiveStatus} initialValue={false} color="success" labelStyle={{ color: "white" }} checkboxStyle={{ margin: 10 }} label={Language.driverWorkingStatus} />
            </Block>
          </Block>
        )
      }
  ...
  return (
    <Block flex center style={styles.home}>
      ...
      <Toast position='top' positionValue={20} ref={toastok} style={{ backgroundColor: argonTheme.COLORS.SUCCESS, }} />
